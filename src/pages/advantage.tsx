...
const Advantage = () => {
  ...
  const firstRowContent: Array<string> = [
    ...
    '實現 NOx、SO2 、Hg 等污染物的氧化吸收，真正實現“一塔多脫”。在氧化氮氧化物之後，臭氧可以繼續氧化 SO 2 、Hg等污染物，並利用歐珊 Wet Scrubbing System 進行排除。無須購置其他脫除污染的設備，真正實現“一塔多脫”。節能且環保的工業技術，一直是歐珊所堅持的理念。',
  ]
  ...
  const secondRowContent: Array<string> = [
    ...
    '臭氧與污染物皆接觸面積大、反應迅速。歐珊使用濕式脫硝洗脫技術，使用大小固定的水滴來獲得有效氣體接觸。簡單的旋風分離法可輕鬆去除任何氣體中的多餘水分。滴灌、噴嘴和液滴分離器是開放式的，非常耐用，連續運行多年不停機。',
  ...
  const thirdRowContent: Array<string> = [
    '臭氧發生系統僅需對部分空氣或氧氣進行放電，即可以完成臭氧生成。歐珊電極的每個電極由高壓電極、不銹鋼網和電介質玻璃管組成放置在接地管內。臭氧在內部和外部的玻璃間隙中產生。能源使用效率極高。',
