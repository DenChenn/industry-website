...
const Generator = () => {
  ...
  const section4Content =
    'WEDECO PDO/PDA 系列臭氧發生器無疑是全球需要可靠、高效地生產大量臭氧設計工程師和用戶的最佳選擇。因為 PDO/PDA 系列發生器具有最精簡、最可靠的、最具成本效益的工業技術設計。更甚者其高運行效率、穩定性、“即插即用”系統(完全儀表化)、操作簡單、堅固的設計(允許在惡劣環境條件下運行)、低維護成本，都是重視效率的客戶的不二之選'
  const section6Title = '歐珊電極技術:'
  const section6Content =
    '採用最新歐珊電極科技。降低能源成本，減少機器所佔用空間、以及簡單的介面供歐珊臭氧系統操作員使用。歐珊電極技術電極技術精要在於:臭氧發生器容器內部，不銹鋼鋼管焊接在兩個固管板之間，作為接地電極。我們的每個電極由高壓電極、不銹鋼網和電介質玻璃管組成放置在接地管內。臭氧在內部和外部的玻璃間隙中產生。'
